...
async def test_llm_github_issue_creation():
    ...
    try:
        ...
        repository_context = RepositoryContext(
            repo=repo, owner=owner, service="github", ref="main"
        )
        ...
        github_tool_functions = [
            f for f in github_functions if f.get("name", "").startswith("create_github")
        ]
                ...
                if call.get("result"):
                    try:
                        ...
                        if call_result.get("success"):
                            ...
                            print(
                                f"      💬 メッセージ: {call_result.get('message', 'N/A')}"
                            )
                        else:
                            print(
                                f"      ❌ エラー: {call_result.get('error', 'Unknown error')}"
                            )
...
async def test_llm_github_permissions_check():
    ...
    try:
        ...
        permissions_function = [
            f
            for f in github_functions
            if f.get("name") == "check_github_repository_permissions"
        ]
                    ...
                    try:
                        ...
                        if call_result.get("success"):
                            ...
                            if "issue_info" in call_result:
                                ...
                                print(
                                    f"      ✅ README改善Issue #{issue_info.get('number')} 作成成功"
                                )
                                print(
