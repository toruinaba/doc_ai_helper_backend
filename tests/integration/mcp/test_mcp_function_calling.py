...
class TestMCPFunctionCallingIntegration:
    ...
    async def test_llm_mcp_tool_integration_mock(self, mcp_server: DocumentAIHelperMCPServer):
    ...
    async def test_llm_mcp_tool_integration_openai(
        ...
        openai_model = os.getenv("OPENAI_MODEL", "gpt-3.5-turbo")  # デフォルトはgpt-3.5-turbo
...
analyze_document_structure関数を使用して分析を行ってください。"""
    ...
    async def test_function_registry_mcp_integration(self, mcp_server: DocumentAIHelperMCPServer):
        ...
        assert registered_functions["extract_document_context"] == extract_document_context
    ...
    async def test_mcp_function_calling_error_handling(self, mcp_server: DocumentAIHelperMCPServer):
    ...
    async def test_mcp_async_function_execution(self, mcp_server: DocumentAIHelperMCPServer):
        """MCPツールの非同期実行をテスト。"""
        import asyncio
