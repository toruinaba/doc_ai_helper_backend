...
class TestMCPPerformanceIntegration:
    ...
    async def test_tool_execution_performance(self, mcp_server: DocumentAIHelperMCPServer):
        """個別ツールの実行時間をテスト。"""
        ...
        assert calc_time < 1.0, f"Calculate execution time {calc_time:.2f}s exceeded 1.0s"
        ...
        print(f"Tool execution times - Calculate: {calc_time:.3f}s, Document: {doc_time:.3f}s")

    async def test_concurrent_tool_execution_performance(self, mcp_server: DocumentAIHelperMCPServer):
        """並行ツール実行のパフォーマンスをテスト。"""
        from doc_ai_helper_backend.services.mcp.tools.utility_tools import calculate
        ...
        assert execution_time < 10.0, f"Concurrent execution time {execution_time:.3f}s exceeded 10.0s"
        ...
        print(f"Concurrent performance: {throughput:.2f} tasks/sec, {execution_time:.3f}s total")

    async def test_memory_usage_during_execution(self, mcp_server: DocumentAIHelperMCPServer):
        """ツール実行中のメモリ使用量をテスト。"""
        import gc
        ...
        assert memory_increase < 500, f"Memory increase {memory_increase:.2f}MB exceeded 500MB"
    ...
    async def test_long_running_session_stability(self, mcp_server: DocumentAIHelperMCPServer):
        """長時間セッションの安定性をテスト。"""
        from doc_ai_helper_backend.services.mcp.tools.utility_tools import calculate, format_text
        ...
        print(f"Long-running stability: {successful_operations} successful ops in {duration:.2f}s, {success_rate:.2%} success rate" if total_operations > 0 else f"Long-running stability: No operations executed in {duration:.2f}s")

    async def test_resource_cleanup_after_errors(self, mcp_server: DocumentAIHelperMCPServer):
        ...
        for result in results:
            if isinstance(result, str):
                try:
                    ...
                    if not result_data.get("success", True):  # successがFalseまたは存在しない場合
                        error_count += 1
        ...
        print(f"Resource cleanup test: {error_count} errors handled, {memory_change:.2f}MB memory change")

    @pytest.mark.slow
    async def test_stress_test_mixed_operations(self, mcp_server: DocumentAIHelperMCPServer):
        """混合操作のストレステスト。"""
        from doc_ai_helper_backend.services.mcp.tools.document_tools import analyze_document_structure
        from doc_ai_helper_backend.services.mcp.tools.utility_tools import calculate, format_text
        ...
        # Feedback tasks (20%)
        for i in range(20):
            mixed_tasks.append(generate_feedback_from_conversation(
        ...
        print(f"Stress test: {len(mixed_tasks)} tasks in {execution_time:.2f}s, {success_rate:.2%} success rate")
