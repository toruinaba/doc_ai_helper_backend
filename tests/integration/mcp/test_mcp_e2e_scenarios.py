...
class TestMCPE2EScenarios:
    ...
    @pytest.mark.skipif(
        not os.getenv("GITHUB_TOKEN"), reason="GitHub token not available for E2E test"
    )
    ...
    async def test_mock_feedback_to_github_scenario(
        ...
        with patch(
            "doc_ai_helper_backend.services.github.github_client.GitHubClient"
        ) as mock_client:
    ...
    async def test_concurrent_user_scenario(
        ...
        async def user_session(user_id: int, content: str):
            ...
            return {"user_id": user_id, "results": results, "completed": True}

        # 5つの並行ユーザーセッション
        user_sessions = [user_session(i, sample_markdown_content) for i in range(1, 6)]
    ...
    async def test_error_recovery_scenario(self, mcp_server: DocumentAIHelperMCPServer):
        ...
        print(
            f"Error recovery scenario: {successful_count} successful, {error_count} errors"
        )
