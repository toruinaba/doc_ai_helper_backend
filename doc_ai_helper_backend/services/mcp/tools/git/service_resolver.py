...
class GitServiceResolver:
    ...
    @classmethod
    def register_adapter(cls, service_type: GitServiceType, adapter_class: Type[BaseGitAdapter]) -> None:
        """
        ...
        logger.debug(f"Registered adapter {adapter_class.__name__} for service {service_type.value}")
    ...
    def get_adapter(cls, repository_context: RepositoryContext) -> BaseGitAdapter:
        ...
        try:
            ...
            logger.debug(f"Created adapter {adapter_class.__name__} for {repository_context.full_name}")
            return adapter
