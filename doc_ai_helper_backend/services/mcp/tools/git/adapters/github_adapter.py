...
class GitHubAdapter(BaseGitAdapter):
    ...
    def _initialize_client(self) -> None:
        ...
        if not credentials or not credentials.get("token"):
            raise ValueError(f"No GitHub token found for repository {self.repository_context.full_name}")
        ...
        self.logger.debug(f"Initialized GitHub adapter for {self.repository_context.full_name}")
