...
class ForgejoAdapter(BaseGitAdapter):
    ...
    def _initialize_client(self) -> None:
        ...
        if not credentials or not credentials.get("token"):
            raise ValueError(
                f"No Forgejo token found for repository {self.repository_context.full_name}"
            )
        ...
        self.logger.debug(
            f"Initialized Forgejo adapter for {self.repository_context.full_name}"
        )
    ...
    async def get_repository_info(self) -> GitOperationResult:
        ...
        try:
            ...
            html_url = (
                ...
                or f"{self.repository_context.base_url}/{self.repository_context.owner}/{self.repository_context.repo}"
            )
