...
class RepositoryOperations:
    ...
    async def get_repository_info(self) -> GitOperationResult:
        ...
        try:
            ...
            self.logger.debug(f"Getting repository info for {self.repository_context.full_name}")
            
            result = await adapter.get_repository_info()
            
            if result.success:
                self.logger.debug(f"Successfully retrieved repository info for {self.repository_context.full_name}")
            else:
                self.logger.error(
    ...
    async def list_branches(self) -> GitOperationResult:
        ...
        try:
            ...
            self.logger.debug(f"Listing branches for {self.repository_context.full_name}")
    ...
    async def get_repository_summary(self) -> GitOperationResult:
        ...
        try:
            ...
            summary_data = {
                "repository_info": repo_info_result.data if repo_info_result.success else None,
                "branches": branches_result.data.get("branches", []) if branches_result.success else [],
                "repository_context": {
