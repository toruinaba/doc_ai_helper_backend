...
class MCPGitHubClient(MCPGitClientBase):
    ...
    async def check_repository_permissions(
        ...
        **kwargs
        ...
        try:
            permissions = await self.github_client.check_repository_permissions(repository)
            ...
            raise GitHubException(f"Failed to check GitHub repository permissions: {str(e)}")
