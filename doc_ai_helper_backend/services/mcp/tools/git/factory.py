...
class MCPGitToolsFactory:
    ...
    def _create_github_adapter(cls, config: Dict[str, Any]) -> MCPGitHubAdapter:
        ...
        return MCPGitHubAdapter(
            ...
            **{k: v for k, v in config.items() if k not in ["access_token", "github_token"]}
        )
    ...
    def _create_forgejo_adapter(cls, config: Dict[str, Any]) -> MCPForgejoAdapter:
        ...
        username = (
            ...
            config.get("forgejo_username") or
            os.getenv("FORGEJO_USERNAME")
        )
        
        password = (
            config.get("password") or
            config.get("forgejo_password") or
        ...
        return MCPForgejoAdapter(
            ...
            **{k: v for k, v in config.items() if k not in [
                "base_url", "access_token", "forgejo_token", 
                "username", "forgejo_username", "password", "forgejo_password"
            ]}
        )
    ...
    def register_adapter(
        cls,
        service_type: str,
        adapter_class: Type[MCPGitToolsBase]
    ) -> None:
        """
