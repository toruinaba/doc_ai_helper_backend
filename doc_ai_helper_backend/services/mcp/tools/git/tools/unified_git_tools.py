...
class UnifiedGitTools:
    ...
    def _parse_repository_context(self, repository_context: Union[str, Dict[str, Any]]) -> RepositoryContext:
        """
            ...
            if isinstance(repository_context, str):
                ...
                try:
                    ...
                    if "/" in repository_context:
                        ...
                        raise ValueError(f"Cannot parse repository context: {repository_context}")
            
            elif isinstance(repository_context, dict):
                return RepositoryContext(**repository_context)
            
            else:
                raise ValueError(f"Unsupported repository context type: {type(repository_context)}")
    ...
    async def create_feature_request(
        ...
        try:
            ...
            self.logger.info(f"Creating feature request '{feature_title}' in {repo_context.full_name}")
    ...
    async def get_repository_summary(
        ...
        try:
            ...
            self.logger.debug(f"Getting repository summary for {repo_context.full_name}")
    ...
    def validate_repository_context(self, repository_context: Union[str, Dict[str, Any]]) -> Dict[str, Any]:
        """
